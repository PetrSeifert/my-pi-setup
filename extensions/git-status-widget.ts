...
async function getUnstagedCount(cwd: string) {
  const status = await runGit(["-c", "core.fsmonitor=false", "status", "--porcelain", "--untracked-files=normal"], cwd);
...
async function updateWidget(ctx: ExtensionContext) {
  ...
  try {
    const insideWorkTree = await runGit(["rev-parse", "--is-inside-work-tree"], ctx.cwd);
