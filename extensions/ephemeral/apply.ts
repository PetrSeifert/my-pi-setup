...
export async function applySelection(
  ...
  for (const key of setDifference(desiredKeys, currentKeys)) {
    const item = catalog.byKey.get(key);
    if (item) additions.push(item);
  }

  const removals: ManifestEntry[] = [];
  for (const key of setDifference(currentKeys, desiredKeys)) {
...
function getRemovalConflict(entry: ManifestEntry, state: ProjectState): string | undefined {
  const unsafeTarget = getUnsafeManagedTargetPath(entry, state);
  if (unsafeTarget) {
    return `managed ${entry.type} '${entry.label}' has unsafe target path: ${unsafeTarget}`;
  }

  const unsafeSetting = getUnsafeSettingsChange(entry, state);
  if (unsafeSetting) {
    return `managed ${entry.type} '${entry.label}' has unsafe settings change: ${unsafeSetting}`;
  }
...
async function removeManagedEntry(entry: ManifestEntry, state: ProjectState): Promise<void> {
  switch (entry.type) {
    ...
    case "prompt":
    case "extension": {
      const targetPaths = getValidatedManagedTargetPaths(entry, state);
      await Promise.all(targetPaths.map((targetPath) => rm(targetPath, { recursive: true, force: true })));

      if (entry.settingsChanges.length === 0) {
        break;
      }

      const settingsPath = getValidatedSettingsChangePath(entry, state);
...
function getValidatedManagedTargetPaths(entry: ManifestEntry, state: ProjectState): string[] {
  const unsafe = getUnsafeManagedTargetPath(entry, state);
  if (unsafe) {
    throw new Error(`Refusing to remove managed ${entry.type} '${entry.id}' with unsafe target path: ${unsafe}`);
  }
  return entry.targetPaths.map((targetPath) => resolve(targetPath));
}

function getUnsafeManagedTargetPath(entry: ManifestEntry, state: ProjectState): string | undefined {
  if (entry.type === "mcp") return undefined;
  if (!Array.isArray(entry.targetPaths) || entry.targetPaths.length === 0) {
    return "missing targetPaths";
  }

  const allowedRoots = getAllowedManagedTargetRoots(entry, state);
  for (const targetPath of entry.targetPaths) {
    if (typeof targetPath !== "string" || targetPath.length === 0) {
      return String(targetPath);
    }
    if (!allowedRoots.some((root) => isPathStrictlyInside(root, targetPath))) {
      return targetPath;
    }
  }
  return undefined;
...
function getAllowedManagedTargetRoots(entry: ManifestEntry, state: ProjectState): string[] {
  switch (entry.type) {
    case "skill":
      return [resolveProjectPath(state.cwd, ".pi", "skills")];
    case "prompt":
      return [resolveProjectPath(state.cwd, ".pi", "prompts")];
    case "extension":
      return [
        resolveProjectPath(state.cwd, ".pi", "extensions"),
        resolveProjectPath(state.cwd, ".pi", "ephemeral", "extensions"),
      ];
    case "mcp":
      return [];
  }
...
function getValidatedSettingsChangePath(entry: ManifestEntry, state: ProjectState): string {
  const unsafe = getUnsafeSettingsChange(entry, state);
  if (unsafe) {
    throw new Error(`Refusing to update settings for managed ${entry.type} '${entry.id}' with unsafe settings change: ${unsafe}`);
  }
  return state.paths.settingsPath;
}

function getUnsafeSettingsChange(entry: ManifestEntry, state: ProjectState): string | undefined {
  if (!Array.isArray(entry.settingsChanges)) {
    return "missing settingsChanges";
  }
  ...
  for (const change of entry.settingsChanges) {
    ...
    if ((change as { kind?: unknown }).kind !== "extensions") {
      return `unsupported kind ${String((change as { kind?: unknown }).kind)}`;
    }

    const file = (change as { file?: unknown }).file;
    if (typeof file !== "string" || resolve(file) !== resolve(state.paths.settingsPath)) {
      return `unexpected file ${String(file)}`;
    }
...
function setDifference<T>(left: Set<T>, right: Set<T>): T[] {
