...
async function scanSkillDirectory(directory: string): Promise<EntryScanResult<SkillCatalogItem>> {
  ...
  try {
    ...
    const skillId = basename(directory);
    if (!isSafePathSegment(skillId)) {
      return { warnings: [`Skipping skill with unsafe directory name: ${directory}`] };
    }

    const skillLabel = frontmatter.name?.trim() || skillId;
    const description = frontmatter.description?.trim() || getFirstNonEmptyLine(body);

    return {
      item: {
        key: resourceKey("skill", skillId),
        id: skillId,
        label: skillLabel,
        description,
        type: "skill",
        ...
        installMode: "copy",
        catalogFingerprint,
        previewTitle: skillLabel,
        previewFormat: "markdown",
        previewContent: content,
        sortText: skillLabel.toLowerCase(),
      },
      warnings: [],
    };
  } catch {
...
async function scanPromptFile(file: string): Promise<EntryScanResult<PromptCatalogItem>> {
  try {
    ...
    if (!isSafePathSegment(fileName)) {
      return { warnings: [`Skipping prompt with unsafe file name: ${file}`] };
    }
...
async function scanSingleFileExtension(file: string): Promise<EntryScanResult<ExtensionCatalogItem>> {
  ...
  if (!isSafePathSegment(entryName)) {
    return { warnings: [`Skipping extension with unsafe file name: ${file}`] };
  }
...
async function scanPackageExtensionDirectory(directory: string): Promise<EntryScanResult<ExtensionCatalogItem>> {
  ...
  if (!isSafePathSegment(entryName)) {
    return { warnings: [`Skipping extension package with unsafe directory name: ${directory}`] };
  }
